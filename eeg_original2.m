...
% Apply Traditional Highpass Filter for Comparison ---
fc = 4; % Cutoff frequency 4 Hz
[b_filt, a_filt] = butter(4, fc / (fs/2), 'high');
filt_sig = filtfilt(b_filt, a_filt, contam_sig);

% Compute PSD using Welch's method for all three signals
[pxx_c, f_psd] = pwelch(contam_sig, [], [], [], fs);
[pxx_r, ~]     = pwelch(recov_sig, [], [], [], fs);
[pxx_f, ~]     = pwelch(filt_sig, [], [], [], fs); % NEW

% Convert to Decibels (dB/Hz)
psd_c_dB = 10*log10(pxx_c);
psd_r_dB = 10*log10(pxx_r);
psd_f_dB = 10*log10(pxx_f); % NEW
...
% Determine Y-axis limits dynamically based on all three data sets
ymin = min([psd_c_dB; psd_r_dB; psd_f_dB]) - 5;
ymax = max([psd_c_dB; psd_r_dB; psd_f_dB]) + 5;
...
% 2. Plot the PSD Lines on top
plot(f_psd, psd_c_dB, 'k', 'LineWidth', 1.2, 'DisplayName', 'Contaminated EEG');
plot(f_psd, psd_r_dB, 'b', 'LineWidth', 1.2, 'DisplayName', 'HO-MSSA Denoised');
plot(f_psd, psd_f_dB, 'r', 'LineWidth', 1.2, 'DisplayName', 'Naive Highpass (fc=4Hz)'); % NEW
...
grid on;
xlim([0 100]); % Cap X-axis at 100Hz
ylim([ymin ymax]); 
legend('Location', 'northeast', 'FontSize', 14);
hold off;

%% 5. Plot Time-Domain Signals (Three Results)
fprintf('Generating Time-Domain plot...\n');

% Create a time vector in seconds
t = (0:length(contam_sig)-1) / fs; 

figure('Name', 'Time-Domain Comparison', 'Position', [150 150 900 650]);

% 1. Contaminated Signal
subplot(3, 1, 1);
plot(t, contam_sig, 'k', 'LineWidth', 1);
title('Contaminated EEG Signal', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('Amplitude');
grid on;
xlim([t(1) t(end)]);

% 2. HO-MSSA Denoised Signal
subplot(3, 1, 2);
plot(t, recov_sig, 'b', 'LineWidth', 1);
title('HO-MSSA Denoised Signal', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('Amplitude');
grid on;
xlim([t(1) t(end)]);

% 3. Traditional Highpass Filtered Signal
subplot(3, 1, 3);
plot(t, filt_sig, 'r', 'LineWidth', 1);
title('Traditional Highpass Filter (fc = 4 Hz)', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('Time (seconds)');
ylabel('Amplitude');
grid on;
xlim([t(1) t(end)]);